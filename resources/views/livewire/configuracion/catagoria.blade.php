<div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm text-gray-600">
                    Agregar categorias, editar y más.
                </p>
            </div>
            <div>
        ...
        @if ($categorias->count())
                    ...
                    @foreach ($categorias as $categoria)
                        <tr>
                            <td class="h-px w-72 whitespace-nowrap">
                                <div class="px-6 py-3">
                                    <span class="block text-sm text-gray-500">{{ $categoria->nombre }}</span>
                                </div>
                            </td>
                            <td class="h-px w-72 whitespace-nowrap">
                                <div class="px-6 py-3">
                                    <span class="block text-sm text-gray-500">{{ $categoria->descripcion }}</span>
                                </div>
                            </td>
                            <td class="size-px whitespace-nowrap">
                                <div class="px-6 py-3">
                                    <span class="text-sm text-gray-500">{{ $categoria->created_at->toFormattedDateString() }}</span>
                                </div>
                            </td>
                            <td class="size-px whitespace-nowrap">
                                <div class="flex">

                                    <a wire:click="edit({{ $categoria->id }})" class="cursor-pointer bg-gray-100 items-center py-2 px-2 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 mr-2">
                                        <i class="fa-duotone fa-pen-to-square"></i>
                                    </a>
                                    <a wire:click="confirmDeleteCategoria({{ $categoria->id }})" class=" items-center py-2 px-2 bg-gray-100 rounded-lg text-sm text-red-600 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 cursor-pointer">
            ...
            <div class="py-2 px-4 float-right">
                <span class="pagination">
                    {{ $categorias->links('vendor.livewire.simple-tailwind') }}
    ...
    <x-dialog-modal wire:model="confirmCategoriaDeletion">
        <x-slot name="title">
            {{ __('Delete User') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4" x-data="{}"
                ...
                <x-input type="password" class="mt-1 block w-3/4" placeholder="{{ __('Password') }}"
                    x-ref="password" wire:model.defer="password" wire:keydown.enter="deleteCategoria" />
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmCategoriaDeletion')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            <x-danger-button class="ml-3" wire:click="deleteCategoria" wire:loading.attr="disabled">
                {{ __('Delete User') }}
            </x-danger-button>
        </x-slot>
    </x-dialog-modal>
