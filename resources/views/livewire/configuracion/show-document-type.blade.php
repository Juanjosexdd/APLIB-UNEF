<div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm text-gray-600">
                    Agregar tipos de documentos, editar y más.
                </p>
            </div>
            <div>
        ...
        @if ($document_types->count())
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-start cursor-pointer" wire:click="order('abbreviation')">
                            <div class="flex items-center gap-x-2 justify-between">
                                <span class="text-xs font-semibold uppercase tracking-wide text-gray-800">
                                    Abreviatura
                                </span>
                                <div class="text-xs font-semibold">
                                    @if ($sort == 'abbreviation')
                                        @if ($direction == 'asc')
                                            <i class="fa-duotone fa-arrow-up-a-z"></i>
                                        @else
                                            <i class="fa-duotone fa-arrow-down-z-a"></i>
                        ...
                        <tr>
                            <td class="h-px w-72 whitespace-nowrap">
                                <div class="px-6 py-3">
                                    ...
                                    <span class="block text-sm text-gray-500">{{ $document_type->name }}</span>
                                </div>
                            </td>
                            <td class="size-px whitespace-nowrap">
                                <div class="px-6 py-3">
                                    <span class="block text-sm text-gray-500">{{ $document_type->abbreviation }}</span>
                                </div>
                            </td>
                            <td class="size-px whitespace-nowrap">
                                <div class="px-6 py-3">
                                    <span
                                        class="text-sm text-gray-500">{{ $document_type->created_at->toFormattedDateString() }}</span>
                            ...
                            <td class="size-px whitespace-nowrap">
                                <div class="flex">
                                    ...
                                    <a wire:click="edit({{ $document_type->id }})" class="cursor-pointer bg-gray-100 items-center py-2 px-2 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 mr-2">
                                        <i class="fa-duotone fa-pen-to-square"></i>
                                    </a>
                                    <a wire:click="confirmDeleteDocumentType({{ $document_type->id }})" class=" items-center py-2 px-2 bg-gray-100 rounded-lg text-sm text-red-600 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 cursor-pointer">
            ...
            <div class="py-2 px-4 float-right">
                <span class="pagination">
                    {{ $document_types->links('vendor.livewire.simple-tailwind') }}
        ...
        <x-dialog-modal wire:model="openCreate">
            ...
            <x-slot name="content">
                <div class="grid grid-cols-2 grid-rows-1 gap-4">
                    ...
                    <div>
                        <x-label for="abbreviation" class="text-sm" value="{{ __('abbreviation') }}" />
                        <x-input id="abbreviation" type="text" class="!py-1 !px-1.5 mt-1 block w-full"
    ...
    <x-dialog-modal wire:model="confirmingDocumentTypeDeletion">
        <x-slot name="title">
            {{ __('Delete User') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4" x-data="{}"
                ...
                <x-input type="password" class="mt-1 block w-3/4" placeholder="{{ __('Password') }}"
                    x-ref="password" wire:model.defer="password" wire:keydown.enter="deleteDocumentType" />
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmingDocumentTypeDeletion')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            <x-danger-button class="ml-3" wire:click="deleteDocumentType" wire:loading.attr="disabled">
                {{ __('Delete User') }}
            </x-danger-button>
        </x-slot>
    </x-dialog-modal>
